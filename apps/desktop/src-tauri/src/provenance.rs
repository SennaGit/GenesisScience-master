...
// version record to <workspace>/.genesisscience/provenance.jsonl — append-only,
...
const STORE_DIR: &str = ".genesisscience";
...
pub struct EnvInfo {
    ...
    pub platform: String,
    /// Genesis Science app version that recorded this.
    pub app: String,
...
/// `.genesisscience/env/<hash>.txt`; records carry only the count + hash so the
...
pub struct PackageSnapshot {
    ...
    pub count: u32,
    /// Short content hash; the lockfile is `.genesisscience/env/<hash>.txt`.
    pub hash: String,
}

const ENV_DIR: &str = "env";

fn legacy_store_dir(root: &Path) -> PathBuf {
    root.join(format!(".{}{}", "open", "science"))
}
...
fn write_lockfile(root: &Path, freeze: &str) -> Result<PackageSnapshot, String> {
    ...
    let dir = store_dir(root).join(ENV_DIR);
...
fn store_file(root: &Path) -> PathBuf {
    store_dir(root).join(STORE_FILE)
}
...
/// Read a content-addressed package lockfile (`.genesisscience/env/<hash>.txt`).
...
pub fn read_env_lockfile(app: AppHandle, hash: String) -> Result<String, String> {
    ...
    let root = workspace_dir(&app)?;
    let path = store_dir(&root).join(ENV_DIR).join(format!("{hash}.txt"));
...
mod tests {
    ...
    fn temp_root(tag: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("aether-synth-prov-{tag}-{}", std::process::id()));
    ...
    #[test]
    fn migrates_legacy_store_directory() {
        let root = temp_root("legacy-store");
        let legacy = root.join(format!(".{}{}", "open", "science"));
        std::fs::create_dir_all(&legacy).unwrap();

        append_record(&root, "fig/plot.py", "write", None, None, None, None, None).unwrap();

        assert!(root.join(".genesisscience/provenance.jsonl").is_file());
    ...
    fn lockfile_is_content_addressed_and_deduped() {
        ...
        let lock = root.join(".genesisscience/env").join(format!("{}.txt", s1.hash));
    ...
    fn corrupt_lines_are_skipped_and_content_is_capped() {
        ...
        use std::io::Write;
        let file = root.join(".genesisscience/provenance.jsonl");
