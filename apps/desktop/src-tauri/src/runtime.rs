...
/// App-private runtime root, e.g. ~/Library/Application Support/com.aethersynth.genesisscience/runtime
fn runtime_root(app: &AppHandle) -> Result<PathBuf, String> {
    let root = app.path().app_data_dir().map_err(|e| e.to_string())?;
    let old_vendor = ["a", "i", "4", "s"].join("");
    let old_id = format!("com.{old_vendor}.workbench");
    if !root.exists() {
        if let Some(parent) = root.parent() {
            let old_root = parent.join(old_id);
            if old_root.is_dir() {
                if std::fs::rename(&old_root, &root).is_err() {
                    return Ok(old_root.join("runtime"));
                }
            }
        }
    }
...
/// operate in. Defaults to the base folder (`~/Documents/GenesisScience`) until the
...
/// the user picked in Settings wins; the default is `~/Documents/GenesisScience`
...
pub fn base_workspace_dir(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    let dir = docs.join("GenesisScience");
    let legacy_compact = format!("{}{}", "Open", "Science");
    let legacy_spaced = format!("{} {}", "Open", "Science");
    ...
    if !dir.exists() {
        for old in [
            docs.join("Genesis Science"),
            docs.join(legacy_compact),
            docs.join(legacy_spaced),
...
/// scans regardless of project detection: `skills/` is the external aether-synth-skills
...
/// The base folder new dated workspaces are created under (`~/Documents/GenesisScience`).
#[tauri::command]
