...
// through <workspace>/.genesisscience/hpc.json.
...
const CONFIG_DIR: &str = ".genesisscience";
...
fn config_path(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    Ok(config_dir(&root).join(CONFIG_FILE))
}
...
/// `<workspace>/.genesisscience/hpc.json`, where the hpc-slurm skill reads it.
#[tauri::command]
...
#[cfg(test)]
mod tests {
    use super::{config_dir, is_safe_host, is_safe_job_id, parse_squeue, parse_ssh_hosts};

    #[test]
    fn migrates_legacy_config_directory() {
        let root = std::env::temp_dir().join(format!("aether-synth-hpc-legacy-{}", std::process::id()));
        ...
        assert_eq!(dir, root.join(".genesisscience"));
        assert!(dir.is_dir());
