...
mod tests {
    ...
    fn list_dir_sorts_dirs_first_and_skips_hidden() {
        let root = std::env::temp_dir().join(format!("aether-synth-listdir-{}", std::process::id()));
    ...
    fn unique_name_dedupes_with_numeric_suffix() {
        let dir = std::env::temp_dir().join(format!("aether-synth-unique-test-{}", std::process::id()));
    ...
    fn locate_finds_literal_bare_and_missing_paths() {
        let root = std::env::temp_dir().join(format!("aether-synth-locate-test-{}", std::process::id()));
    ...
    fn locate_prefers_the_newest_of_duplicate_basenames() {
        let root = std::env::temp_dir().join(format!("aether-synth-locate-dup-test-{}", std::process::id()));
