...
[![Genesis Science — 面向科研人员的开源 AI 工作台](./docs/assets/banner.webp)](https://github.com/aether-synth/GenesisScience)
...
- **可复现** —— 代码、数据、图表、报告、日志与 `provenance.jsonl` 全部保留，且每个
  工件版本都可回溯。
- **多学科** —— 从可复现计算科学起步，逐步扩展到材料、化学、生物、医学与工程。
...
| **工件溯源** | 智能体每次写文件都会向 `.genesisscience/provenance.jsonl` 追加版本记录；History 面板展示每个版本的代码、模型与来源对话。 |
...
- `research-explorer`、`literature-survey`、`experiment-suite`、`paper-writer`、
  `mindmap-render`、`integrity-auditor`、`aether-synth-agent` —— 即
  [aether-synth-skills](https://github.com/aether-synth/aether-synth-skills) 技能包。
...
bash scripts/dev/fetch-skills.sh     # aether-synth-skills 技能包
...
- **添加数据** —— 把文件放进工作区（`~/Documents/GenesisScience`），或在输入框里附加；
  智能体在那里读写。
...
| `runtime/skills/external/` | 内置的 `aether-synth-skills` 技能包（脚本拉取） |
...
[aether-synth-skills](https://github.com/aether-synth/aether-synth-skills) 技能包构建。感谢
