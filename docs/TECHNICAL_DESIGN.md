# Genesis Science Desktop — Technical Design
...
reconsidered. But Genesis Science's core is the workbench, files, agent, runtime, and
...
- with an **app-private** config/data dir via `XDG_CONFIG_HOME`/`XDG_DATA_HOME` under
  `~/Library/Application Support/com.aethersynth.genesisscience/runtime/` (macOS) — so the user's
...
```text
~/.aether-synth-workbench/
  config/  runtime/{opencode,python,node}/  profiles/aether-synth-workbench/
  workspaces/  logs/  cache/  secrets/
```

Windows: `%APPDATA%/Genesis Science/` · macOS: `~/Library/Application Support/Genesis Science/`
...
Outputs: `GenesisScience-aarch64.dmg`, `GenesisScience-x64.dmg`,
`GenesisScience-universal.dmg` (later). Code signing / notarization needs an Apple
Developer account; a free account cannot notarize, so users may still see an
"unverified" prompt.

### 12.2 Windows

Outputs: `GenesisScience-Setup.exe`, `GenesisScience.msi` (later). Prefer the NSIS
...
- `runtime/opencode-profile` — the Genesis Science OpenCode config/skills bundle.
