# Genesis Science — Prioritized Requirements (Community-Informed)
...
> prioritized requirements for **Genesis Science**, our open-source alternative.
...
Genesis Science must win on the same three axes — plus two things Claude Science is
...
Market as: *"Genesis Science — an open AI research workbench for reproducible
...
- **Status.** Shipped: every agent write appends a version record to
  `.genesisscience/provenance.jsonl` (code, tool, model, session, timestamp, and
  ...
  `.genesisscience/env/<hash>.txt` (identical environments dedupe to one file);
...
- **Status.** Shipped: one documented chart design system — a validated
  ...
  three places kept in sync (`@aether-synth/shared` chartPalette, `index.css --series-*`,
  and `runtime/.../genesisscience.mplstyle` applied by the `publication-figures`
